#!/bin/bash
# Helper script to record DockUp demo using different tools
# Usage: ./scripts/record-demo.sh [tool] [output-file]
...
echo -e "${BLUE}🎬 DockUp Demo Recording${NC}"
echo ""
...
case "$TOOL" in
    vhs)
        ...
        echo -e "${YELLOW}Note: Edit scripts/demo.tape to customize the demo${NC}"
        vhs scripts/demo.tape
        if [ -f "demo.gif" ]; then
            mv demo.gif "$OUTPUT"
            echo -e "${GREEN}✅ Demo saved to: $OUTPUT${NC}"
        fi
        ;;
